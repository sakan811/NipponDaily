<template>
  <div
    data-testid="summary-fallback-container"
  ...
  >
    ...
    >
      <!-- Warning Header -->
      <div class="flex items-start gap-4 pb-4 border-b border-stone-200 dark:border-stone-800">
        <div class="w-10 h-10 rounded-lg bg-amber-500/10 text-amber-500 flex items-center justify-center shrink-0 mt-0.5">
          <UIcon name="i-heroicons-exclamation-triangle" class="w-6 h-6 animate-pulse" />
        </div>
        <div class="space-y-1 flex-1">
          <div class="flex items-center gap-2 flex-wrap">
            <h3 class="text-xl font-serif font-bold text-stone-900 dark:text-white">
          ...
          <p class="text-xs text-stone-500 dark:text-stone-400 leading-relaxed">
            The AI summary process encountered a rate limit or service delay. Below are the unsummarized raw sources retrieved for this topic.
          </p>
        </div>
      </div>
      ...
      <div class="space-y-2">
        ...
        <p class="text-xs text-stone-600 dark:text-stone-300 italic">
          "Full AI thematic analysis and executive summary bullet points could not be generated at this time."
        </p>
      </div>

      <!-- Raw Sources List -->
      <div v-if="sources && sources.length > 0" class="space-y-3 pt-2">
        <h5 class="text-xs font-bold text-stone-500 dark:text-stone-400 uppercase tracking-wider flex items-center gap-1.5">
        ...
        <div class="space-y-2">
          ...
          >
            <div class="flex items-start gap-2.5 min-w-0">
              <span class="text-xs font-mono text-stone-400 shrink-0">[{{ idx + 1 }}]</span>
              <div class="min-w-0">
                <a
                  v-if="src.url"
                  :href="src.url"
                ...
                <span v-else class="text-sm font-bold text-stone-900 dark:text-white line-clamp-1">
                  {{ src.title }}
                </span>
      ...
      <!-- Action Button -->
      <div class="pt-4 flex flex-wrap gap-3 items-center justify-between border-t border-stone-200 dark:border-stone-800">
    ...
    >
      <div class="flex items-center justify-between">
        ...
        <UBadge color="warning" variant="soft" size="xs">Testing & Design Mode</UBadge>
      </div>
      <p class="text-xs text-stone-500 dark:text-stone-400">
        This demonstrates the Summary Fallback component layout when Gemini API summarization fails or encounters rate limits.
      </p>
    </div>
  </div>
</template>
