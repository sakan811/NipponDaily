...
const diagramId = props.id || `mermaid-${Math.random().toString(36).substr(2, 9)}`
...
const renderDiagram = async () => {
  ...
  try {
    ...
    const svgElementContainer = mermaidOutput.value.querySelector(`#${diagramId}-svg-container`)
...
<template>
  <div class="mermaid-container mx-auto max-w-3xl">
    <div class="relative group rounded-xl border border-gray-200 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-900/50 backdrop-blur-sm overflow-hidden shadow-sm transition-all duration-300 hover:shadow-md hover:border-primary-500/30">
      <!-- Diagram Area -->
      <div ref="container" class="w-full h-[500px] cursor-grab active:cursor-grabbing relative">
        <div v-if="isLoading" class="absolute inset-0 z-10 flex flex-col items-center justify-center bg-gray-50/50 dark:bg-gray-900/50 backdrop-blur-sm text-gray-400 gap-3">
        ...
        <div ref="mermaidOutput" class="w-full h-full"></div>
      </div>
      
      <!-- Integrated NuxtUI Controls -->
      <div class="absolute bottom-4 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
        <div class="flex items-center shadow-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 rounded-lg p-0.5">
          <UButton
      ...
      <div class="absolute top-4 right-4 pointer-events-none">
        <UKbd size="md" class="opacity-40 group-hover:opacity-100 transition-opacity">Scroll to Zoom</UKbd>
      </div>
    </div>
  </div>
</template>
