...
import type { Story, StorySource, StoryLesson } from "~~/types/index";
...
/**
 * Optional Japanese-learning lesson fields, generated by the calling agent the
 * same way summary/thematicAnalysis are. Shared by upsert_story and
 * merge_stories. All optional so old-style calls keep working unchanged.
 */
const lessonInputFields = {
  originalText: z
    .string()
    .optional()
    .describe(
      "A representative passage (a few sentences to a short paragraph) from the Japanese-language source(s) to teach from — not the full article. Omit when no source has substantive Japanese original text.",
    ),
  furiganaText: z
    .string()
    .optional()
  ...
  vocabList: z
    ...
    .optional()
    .describe(
      "8-15 notable terms from the passage, each with reading, meaning, an approximate JLPT level, and the example sentence it appears in.",
    ),
  grammarNotes: z
    .array(grammarNoteSchema)
    .optional()
    .describe(
      "1-3 grammar patterns worth flagging from the passage, each with a plain explanation and an example sentence.",
    ),
  difficultyLevel: jlptLevelSchema
    .optional()
    .describe(
      "One overall JLPT (N5-N1) difficulty estimate for the story as a lesson.",
    ),
};

function pickLessonFields(input: {
  originalText?: string;
  furiganaText?: string;
  vocabList?: unknown[];
  ...
  if (input.originalText !== undefined)
    lesson.originalText = input.originalText;
  if (input.furiganaText !== undefined)
    lesson.furiganaText = input.furiganaText;
  if (input.vocabList !== undefined)
    lesson.vocabList = input.vocabList as StoryLesson["vocabList"];
  if (input.grammarNotes !== undefined)
    lesson.grammarNotes = input.grammarNotes as StoryLesson["grammarNotes"];
  if (input.difficultyLevel !== undefined)
    lesson.difficultyLevel =
      input.difficultyLevel as StoryLesson["difficultyLevel"];
  return lesson;
}
  ...
  (server) => {
    ...
    server.registerTool(
      ...
      async ({
        ...
        // Lesson fields: use what was submitted; on an extend, keep the
        // existing value for anything not resubmitted this call.
        const submittedLesson = pickLessonFields(lessonInput);
        const lesson: StoryLesson = existing
          ? {
              originalText:
                submittedLesson.originalText ?? existing.originalText,
              furiganaText:
                submittedLesson.furiganaText ?? existing.furiganaText,
              vocabList: submittedLesson.vocabList ?? existing.vocabList,
              grammarNotes:
                submittedLesson.grammarNotes ?? existing.grammarNotes,
              difficultyLevel:
                submittedLesson.difficultyLevel ?? existing.difficultyLevel,
            }
          : submittedLesson;
    ...
    server.registerTool(
      ...
      async ({
        ...
        const lesson: StoryLesson = {
          originalText:
            submittedLesson.originalText ??
            found.find((s) => s.originalText)?.originalText,
          furiganaText:
            submittedLesson.furiganaText ??
            found.find((s) => s.furiganaText)?.furiganaText,
          vocabList:
            submittedLesson.vocabList ??
            found.find((s) => s.vocabList?.length)?.vocabList,
          grammarNotes:
            submittedLesson.grammarNotes ??
            found.find((s) => s.grammarNotes?.length)?.grammarNotes,
          difficultyLevel:
            submittedLesson.difficultyLevel ??
            found.find((s) => s.difficultyLevel)?.difficultyLevel,
        };
