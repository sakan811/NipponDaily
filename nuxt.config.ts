...
export default defineNuxtConfig({
  ...
  nitro: {
    // Embeds kuromoji's dictionary (see server/utils/tokenizer.ts) into the
    // server build itself instead of relying on it being traced and copied
    // as loose files — serverless builds (e.g. Vercel) only deploy files
    // reachable from the require/import graph.
    serverAssets: [{ baseName: "kuromoji-dict", dir: kuromojiDictDir }],
  },
});
