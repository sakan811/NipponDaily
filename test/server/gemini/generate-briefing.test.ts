...
describe("GeminiService", () => {
  ...
  it("returns fallback briefing when API key is missing", async () => {
    const newsItems = [
      { id: "1", title: "Test", summary: "", content: "", category: "Technology" as any, source: "NHK", url: "https://nhk.jp", publishedAt: "2024-01-01" }
    ];
  ...
  it("handles successful AI generation", async () => {
    ...
    const result = await service.generateNewsBriefing(newsItems, { apiKey: "test-key" });
  ...
  it("falls back to empty response if generation fails", async () => {
    ...
    const newsItems = [
      { id: "1", title: "Test Error", summary: "", content: "", category: "Technology" as any, source: "Unknown", url: "https://example.com", publishedAt: "2024-01-01" }
    ];
