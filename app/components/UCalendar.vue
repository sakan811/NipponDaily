<template>
  <div class="u-calendar bg-white dark:bg-stone-900 select-none p-3 max-w-full">
    ...
    <div class="flex items-center justify-between mb-4">
      ...
      />
      <span
        class="text-sm font-serif font-bold text-stone-850 dark:text-stone-150"
      >
    ...
    <div class="flex flex-col md:flex-row gap-6">
      ...
      <div class="flex-1">
        <div
          class="text-center font-serif font-bold text-xs mb-2 text-stone-500"
        >
          {{ getMonthName(navMonth) }} {{ navYear }}
        </div>
        <div
          class="grid grid-cols-7 gap-1 text-center text-xs font-semibold text-stone-400 mb-1"
        >
          <span
      ...
      <div class="flex-1">
        ...
        <div class="grid grid-cols-7 gap-1 text-center">
          ...
          <button
            ...
            type="button"
            :disabled="
              isDateDisabled(nextMonthData.year, nextMonthData.month, day)
            "
            :class="[
              'w-8 h-8 flex items-center justify-center rounded-lg text-xs font-medium transition-colors cursor-pointer',
...
const props = defineProps<{
  modelValue?:
    | { start: CalendarDate | null; end: CalendarDate | null }
    | CalendarDate
    | null;
...
const month1Days = computed(() =>
  getDaysInMonth(navYear.value, navMonth.value),
);
const month1Blanks = computed(() =>
  getFirstDayOfMonth(navYear.value, navMonth.value),
);

const month2Days = computed(() =>
  getDaysInMonth(nextMonthData.value.year, nextMonthData.value.month),
);
const month2Blanks = computed(() =>
  getFirstDayOfMonth(nextMonthData.value.year, nextMonthData.value.month),
);
...
const toMs = (y: number, m: number, d: number) =>
...
const selectDate = (y: number, m: number, d: number) => {
  ...
  if (props.range) {
    const currentModel = props.modelValue as {
    ...
    if (
      !currentModel ||
      !currentModel.start ||
      (currentModel.start && currentModel.end)
    ) {
      emit("update:modelValue", { start: newDate, end: null });
    } else {
      const startMs = toMs(
        currentModel.start.year,
        currentModel.start.month,
        currentModel.start.day,
      );
